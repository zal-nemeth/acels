...
void svd(const double A[45], double U[45], double s[5], double V[25])
{
  ...
  for (q = 0; q < 5; q++) {
    ...
    for (k = qp1; k < 6; k++) {
      ...
      if (apply_transform) {
        blas::xaxpy(9 - q,
                    -(blas::xdotc(9 - q, b_A, qq, b_A, i + 1) / b_A[q + 9 * q]),
                    qq, b_A, i + 1);
      }
      e[k - 1] = b_A[i];
    }
    ...
    if (q + 1 <= 3) {
      ...
      if (nrm == 0.0) {
        ...
        for (k = qp1; k < 6; k++) {
          blas::xaxpy(8 - q, e[k - 1], b_A, (q + 9 * (k - 1)) + 2, work, q + 2);
        }
        for (k = qp1; k < 6; k++) {
          blas::b_xaxpy(8 - q, -e[k - 1] / e[q + 1], work, q + 2, b_A,
                        (q + 9 * (k - 1)) + 2);
        }
      }
      for (k = qp1; k < 6; k++) {
